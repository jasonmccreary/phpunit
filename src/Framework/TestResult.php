 ...
 */
class \PHPUnit\Framework\TestResult implements Countable
{
    /**
     * @var array
     */
     ...
     * @param float                  $time
     */
    public function addError(\PHPUnit\Framework\Test $test, $t, $time)
    {
        if ($t instanceof \PHPUnit\Framework\RiskyTest) {
    ...
    public function addFailure(\PHPUnit\Framework\Test $test, \PHPUnit\Framework\AssertionFailedError $e, $time)
    {
        if ($e instanceof \PHPUnit\Framework\RiskyTest ||
            $e instanceof \PHPUnit\Framework\OutputError) {
    ...
    public function run(\PHPUnit\Framework\Test $test)
    {
        ...
        $collectCodeCoverage = $this->codeCoverage !== null &&
                               !$test instanceof \PHPUnit\Framework\WarningTestCase &&
                               !$coversNothing;
        ...
        try {
            ...
            $this->addFailure(
                $test,
                new \PHPUnit\Framework\RiskyTestError(
                    $e->getMessage()
                ),
                $_timeout
            );
            ...
            $e = new \PHPUnit\Framework\Warning(
                $e->getMessage()
            );
            ...
            if (PHP_MAJOR_VERSION === 7 && $e instanceof \AssertionError) {
                ...
                $e = new \PHPUnit\Framework\AssertionFailedError(
                    sprintf(
                        '%s in %s:%s',
                        $e->getMessage(),
                        $frame['file'],
        ...
        if ($collectCodeCoverage) {
            ...
            try {
                ...
                $this->addFailure(
                    $test,
                    new \PHPUnit\Framework\UnintentionallyCoveredCodeError(
                ...
                $this->addFailure(
                    $test,
                    new \PHPUnit\Framework\CoveredCodeNotExecutedException(
                ...
                if ($linesToBeCovered !== false) {
                    $this->addFailure(
                        $test,
                        new \PHPUnit\Framework\MissingCoversAnnotationException(
        ...
        if ($error === true) {
            ...
            $this->addFailure(
                $test,
                new \PHPUnit\Framework\OutputError(
                    sprintf(
