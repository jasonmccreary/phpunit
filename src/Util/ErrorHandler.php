<?php
namespace PHPUnit\Util\ErrorHandler.php
...
class ErrorHandler
{
    ...
    public static function handleError($errno, $errstr, $errfile, $errline)
    {
        ...
        if ($errno == E_NOTICE || $errno == E_USER_NOTICE || $errno == E_STRICT) {
            if (\PHPUnit\Framework\Error\Notice::$enabled !== true) {
                return false;
            }

            $exception = '\PHPUnit\Framework\Error\Notice';
        } elseif ($errno == E_WARNING || $errno == E_USER_WARNING) {
            if (\PHPUnit\Framework\Error\Warning::$enabled !== true) {
                return false;
            }

            $exception = '\PHPUnit\Framework\Error\Warning';
        } elseif ($errno == E_DEPRECATED || $errno == E_USER_DEPRECATED) {
            if (\PHPUnit\Framework\Error\Deprecated::$enabled !== true) {
                return false;
            }
