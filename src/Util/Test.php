<?php
namespace PHPUnit\Util\Test.php
...
class Test
{
    ...
    public static function describe(\PHPUnit\Framework\Test $test, $asString = true)
    {
        if ($asString) {
            ...
            } elseif ($test instanceof \PHPUnit\Framework\SelfDescribing) {
                return ['', $test->toString()];
            } else {
                return ['', get_class($test)];
            }
    ...
    private static function getLinesToBeCoveredOrUsed($className, $methodName, $mode)
    {
        ...
        if (!empty($annotations['class'][$mode . 'DefaultClass'])) {
            if (count($annotations['class'][$mode . 'DefaultClass']) > 1) {
                throw new \PHPUnit\Framework\CodeCoverageException(
                    sprintf(
    ...
    public static function getDataFromTestWithAnnotation($docComment)
    {
        ...
        if (preg_match(self::REGEX_TEST_WITH, $docComment, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            if (!$data) {
                throw new \PHPUnit\Framework\Exception('The dataset for the @testWith annotation cannot be parsed.');
            }
            ...
            if (isset($methodName[0]) && $methodName[0] == '<') {
                ...
                foreach ($classes as $className) {
                    if (!class_exists($className) &&
                        ...
                        throw new \PHPUnit\Framework\InvalidCoversTargetException(
                            sprintf(
