 ...
 */
abstract class \PHPUnit\Util\PHP
{
    /**
     * @var Runtime
     */
    ...
    public static function factory()
    {
        if (DIRECTORY_SEPARATOR == '\\') {
            return new \PHPUnit\Util\PHP\Windows;
        }

        return new \PHPUnit\Util\PHP\Default;
    }
    ...
    private function processChildResult(\PHPUnit\Framework\Test $test, \PHPUnit\Framework\TestResult $result, $stdout, $stderr)
    {
        ...
        if (!empty($stderr)) {
            $result->addError(
                $test,
                new \PHPUnit\Framework\Exception(trim($stderr)),
                $time
            );
        } else {
            ...
            try {
                ...
                $result->addError(
                    $test,
                    new \PHPUnit\Framework\Exception(trim($stdout), 0, $e),
                    $time
                );
            }
            ...
            if ($childResult !== false) {
                ...
                /* @var $childResult \PHPUnit\Framework\TestResult */
