<?php
namespace PHPUnit\Extensions\GroupTestSuite.php
 ...
 * </code>
 */
class GroupTestSuite extends \PHPUnit\Framework\TestSuite
{
    public function __construct(\PHPUnit\Framework\TestSuite $suite, array $groups)
    {
        $groupSuites = [];
        $name        = $suite->getName();

        foreach ($groups as $group) {
            $groupSuites[$group] = new \PHPUnit\Framework\TestSuite($name . ' - ' . $group);
            $this->addTest($groupSuites[$group]);
        }

        $tests = new RecursiveIteratorIterator(
            new \PHPUnit\Util\TestSuiteIterator($suite),
            RecursiveIteratorIterator::LEAVES_ONLY
        );

        foreach ($tests as $test) {
            if ($test instanceof \PHPUnit\Framework\TestCase) {
                $testGroups = \PHPUnit\Util\Test::getGroups(
                    get_class($test),
                    $test->getName(false)
                );
