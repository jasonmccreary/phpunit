<?php
namespace PHPUnit\TextUI\Command.php
...
class Command
{
     ...
     *
     * @return \PHPUnit\TextUI\TestRunner
     */
...
namespace PHPUnit\TextUI\Command.php
    ...
    protected function handleArguments(array $argv)
    {
        ...
        try {
            $this->options = \PHPUnit\Util\Getopt::getopt(
                $argv,
                'd:c:hv',
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                case '--colors':
                    $this->arguments['colors'] = $option[1] ?: \PHPUnit\TextUI\ResultPrinter::COLOR_AUTO;
                    break;
                ...
                case '--generate-configuration':
                    ...
                    $generator = new \PHPUnit\Util\ConfigurationGenerator;

                    file_put_contents(
                        'phpunit.xml',
                        $generator->generateDefaultConfiguration(
                            \PHPUnit\Runner\Version::series(),
                            $bootstrapScript,
                            $testsDirectory,
                            $src
                        )
                ...
                case '--teamcity':
                    $this->arguments['printer'] = '\PHPUnit\Util\Log\TeamCity';
                    break;
                ...
                case '--atleast-version':
                    exit(version_compare(\PHPUnit\Runner\Version::id(), $option[1], '>=')
                        ? \PHPUnit\TextUI\TestRunner::SUCCESS_EXIT
                        : \PHPUnit\TextUI\TestRunner::FAILURE_EXIT
                    );
                    break;
        ...
        if (isset($this->arguments['test']) && is_string($this->arguments['test']) && substr($this->arguments['test'], -5, 5) == '.phpt') {
            $test = new \PHPUnit\Extensions\PhptTestCase($this->arguments['test']);

            $this->arguments['test'] = new \PHPUnit\Framework\TestSuite;
    ...
    protected function handleLoader($loaderClass, $loaderFile = '')
    {
        if (!class_exists($loaderClass, false)) {
            if ($loaderFile == '') {
                $loaderFile = \PHPUnit\Util\Filesystem::classNameToFilename(
                    $loaderClass
                );
            }
     ...
     *
     * @return \PHPUnit\Util\Printer|string
     */
    protected function handlePrinter($printerClass, $printerFile = '')
    {
        if (!class_exists($printerClass, false)) {
            if ($printerFile == '') {
                $printerFile = \PHPUnit\Util\Filesystem::classNameToFilename(
                    $printerClass
                );
            }
    ...
    protected function handleSelfUpdate($upgrade = false)
    {
        ...
        if (!$upgrade) {
            ...
            $remoteFilename = sprintf(
                'https://phar.phpunit.de/phpunit%s.phar',
                \PHPUnit\Runner\Version::getReleaseChannel()
            );
        }
